...
class LanguageClientErrorHandler {
    ...
    showOnErrorPrompt(): void {
        Telemetry.sendTelemetryEvent('pgSqlToolsServiceCrash');

        // TODO: Show this error for Postgres (github issue #833)
        // // this.vscodeWrapper.showErrorMessage(
        // //   Constants.pgSqlToolsServiceCrashMessage,
        // //   Constants.pgSqlToolsServiceCrashButton).then(action => {
        // //     if (action && action === Constants.pgSqlToolsServiceCrashButton) {
        // //         opener(Constants.pgSqlToolsServiceCrashLink);
        // //     }
        // // });
    }
...
export default class PgSqlToolsServiceClient {
    // singleton instance
    private static _instance: PgSqlToolsServiceClient = undefined;
    ...
    public static get instance(): PgSqlToolsServiceClient {
        if (this._instance === undefined) {
            ...
            this._instance = new PgSqlToolsServiceClient(serviceProvider, logger, statusView);
        }
        return this._instance;
    }

    // initialize the PostgreSQL Tools Service Client instance by launching
            ...
            if (!platformInfo.isValidRuntime()) {
                ...
                this._server.getServerPath(platformInfo.runtimeId, true).then(serverPackagePath => {
                    ...
                    if (serverPackagePath === undefined) {
                        ...
                        this._logger.appendLine("PGSQL Service copied to user local directory");
                    }
                });
            }
        });
     ...
     *
     * @memberOf pgSqlToolsServiceClient
     */
    ...
    private createLanguageClient(serverOptions: ServerOptions): LanguageClient {
        ...
        let client = new LanguageClient(Constants.pgSqlToolsServiceName, serverOptions, clientOptions);
        client.onReady().then( () => {
