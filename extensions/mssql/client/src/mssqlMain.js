...
var dataprotocol_client_1 = require('dataprotocol-client');
var vscode_extension_telemetry_1 = require('vscode-extension-telemetry');
var nls = require('vscode-nls');
var localize = nls.loadMessageBundle();
function activate(context) {
    vscode_1.window.showErrorMessage("mssql extension has been activated");
    ...
    vscode_1.languages.getLanguages().then(function (languageIds) {
        ...
        var serverModule = context.asAbsolutePath(path.join('server', 'out', 'mssqlServerMain.js'));
        // The debug options for the server
        var debugOptions = { execArgv: ['--nolazy', '--debug=6004'] };
        // If the extension is launch in debug mode the debug server options are use
        // Otherwise the run options are used
        var serverOptions = {
            run: { module: serverModule, transport: dataprotocol_client_1.TransportKind.ipc },
            debug: { module: serverModule, transport: dataprotocol_client_1.TransportKind.ipc, options: debugOptions }
        };
        ...
        var client = new dataprotocol_client_1.LanguageClient('json', localize('jsonserver.name', 'JSON Language Server'), serverOptions, clientOptions);
