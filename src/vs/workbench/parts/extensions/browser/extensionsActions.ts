	...
	public run(event: any): TPromise<any> {
		switch (this.contextService.getWorkbenchState()) {
			case WorkbenchState.FOLDER:
				// {{SQL CARBON EDIT}}
				return this.openExtensionsFile(this.contextService.getWorkspace().folders[0].toResource(paths.join('.sqlops', 'extensions.json')));
	...
	public run(): TPromise<any> {
		...
		return pickFolderPromise
			.then(workspaceFolder => {
				if (workspaceFolder) {
					// {{SQL CARBON EDIT}}
					return this.openExtensionsFile(workspaceFolder.toResource(paths.join('.sqlops', 'extensions.json')));
				}
				return null;
			});
	}
