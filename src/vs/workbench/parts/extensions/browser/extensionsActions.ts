...
export class ConfigureWorkspaceRecommendedExtensionsAction extends Action {
	...
	private getOrCreateExtensionsFile(): TPromise<{ created: boolean, extensionsFileResource: URI }> {
		// {{SQL CARBON EDIT}}
		const extensionsFileResource = URI.file(paths.join(this.contextService.getLegacyWorkspace().resource.fsPath, '.carbon', 'extensions.json')); // TODO@Sandeep (https://github.com/Microsoft/vscode/issues/29242)
